> 🧬 **[EvoMate](https://github.com/rainnochen/EvoMate/blob/main/README.md)** — *Agent Digital Genetics Lab*
> [🏠 主页 / Home](https://github.com/rainnochen/EvoMate/blob/main/README.md) | [📖 最新 PRD](https://github.com/rainnochen/EvoMate/blob/main/docs-zh/prd-current.zh-CN.md) | [🏗 架构文档](https://github.com/rainnochen/EvoMate/blob/main/docs-zh/architecture.zh-CN.md) | [📝 开发日志](https://github.com/rainnochen/EvoMate/tree/main/开发日志) | [🤖 OpenClaw 接入](https://github.com/rainnochen/EvoMate/blob/main/docs-zh/openclaw-evomate-integration-prd.zh-CN.md)
---

# EvoMate 当前主 PRD

当前主产品 PRD 版本：

[EvoMate PRD v0.5](./prd-v0.5-openclaw-skill-room-code.zh-CN.md)

## 当前版本说明

`prd-v0.5-openclaw-skill-room-code.zh-CN.md` 是当前最新主产品版本，主要增加了 OpenClaw Gateway 真实网关的接入、Room Code 匹配系统，以及将进化流程封装为 OpenClaw 的原生技能。

它覆盖当前 Demo 的主链路：

```text
Select Arena
→ Evaluate Parent A
→ Evaluate Parent B
→ Calculate Parent Baseline
→ Genome Fusion
→ Mutation
→ Generate Child Candidates
→ Evaluate Child Candidates in Same Arena
→ Select Best Child
→ Compare Child vs Parent Baseline
→ Export Child Genome
→ Evolution Result
```

## 版本关系

- `prd.zh-CN.md`：早期 PRD 草案。
- `prd-v0.1.zh-CN.md`：产品说明书 v0.1 实现版 PRD。
- `prd-v0.2.zh-CN.md`：完整项目逆向 PRD。
- `prd-v0.3.zh-CN.md`：当前最新 Agent Evolution Protocol PRD。
- `prd-v0.4-online-multiplayer.zh-CN.md`：在线多人匹配与技能执行 PRD。
- `prd-v0.5-openclaw-skill-room-code.zh-CN.md`：OpenClaw Gateway 接入与 Room Code 系统 PRD。
- `prd-current.zh-CN.md`：当前主 PRD 指针文件。
- `next-development-prd.zh-CN.md`：后续开发规划，不等同于当前主 PRD。

以后如果出现 `prd-v0.4.zh-CN.md`，只需要更新本文件中的链接即可。
